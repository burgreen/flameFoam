...
:
    ...
    p0_(mesh_.time().value()==0 ?
        ...
        : dimensionedScalar("p0", average(p_))),
        // burgreen restart not working : dimensionedScalar("p0", dimPressure, coeffDict_.lookup<scalar>("p0"))),

    rho0_(mesh_.time().value()==0 ?
        dimensionedScalar("rho0", average(combModel_.rho()))
        : dimensionedScalar("rho0", average(combModel_.rho()))),
        // burgreen restart not working : dimensionedScalar("rho0", dimDensity, coeffDict_.lookup<scalar>("rho0"))),
